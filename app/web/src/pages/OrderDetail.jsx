...
  const getPaymentStatusStyle = (status) => paymentStatusStyles[status] || paymentStatusStyles.default;
  ...
  // Handler to navigate to review page for a specific product
  const navigateToReview = (productId) => {
      // Directs user to the review submission route, passing the product ID
...
  // Determine which status to display in the main header badge
  const mainStatus = isPaid ? order.shipmentStatus : order.paymentStatus;
  
  // Determine the styling based on the primary status
  const mainStatusStyle = isPaid ? getStatusColor(mainStatus) : getPaymentStatusStyle(mainStatus);
  
  // Determine the label text
  const mainStatusText = mainStatus.replace('_', ' ').toUpperCase();
  // --------------------
...
              Order Details
              <span
                className={`inline-block px-4 py-1 rounded-full text-sm font-semibold transition-transform transform hover:scale-105 ${mainStatusStyle}`}
              >
                {mainStatusText}
              </span>
            </h1>
...
              {/* Order Status (Shipment/Delivery) */}
...
                      {order.shipmentStatus.replace('_', ' ').toUpperCase()}
                    </span>
                  </div>
                </div>
                
...
              {/* Order Items - ADD REVIEW BUTTON HERE */}
              <div className="bg-white rounded-lg shadow-sm p-6">
                <h2 className="text-lg font-semibold text-gray-900 mb-4">Order Items</h2>
                <div className="space-y-4">
                  {order.items.map((item, index) => (
                    <div key={index} className="flex items-start space-x-4 border-b border-gray-200 pb-4 last:border-b-0">
                      <img
...
                        
                        {/* Conditional Review Button per Product */}
                        {isDelivered && (
                            <button
                                onClick={() => navigateToReview(item.product._id)}
                                className="mt-2 inline-flex items-center px-3 py-1 bg-yellow-500 text-white text-xs font-medium rounded-full hover:bg-yellow-600 transition-colors shadow-sm"
                            >
