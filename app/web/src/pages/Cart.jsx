...
const Cart = () => {
        ...
        <div className="min-h-screen bg-gray-50">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
            <div className="text-center">
              ...
              <Link to="/products" className="bg-blue-600 text-white px-8 py-3 rounded-lg font-semibold hover:bg-blue-700 transition-colors inline-flex items-center">
                Start Shopping
  ...
  return (
    <>
      <Helmet>
        {/* --- THIS IS THE FIX --- */}
        <title>{`Cart (${itemCount}) - Sportify`}</title>
      </Helmet>
      <div className="min-h-screen bg-gray-50">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="p-6 space-y-6">
                  {filteredItems.map((item) => (
                    <div key={item._id} className="flex items-center space-x-4 border-b border-gray-200 pb-6 last:border-b-0 group hover:bg-gray-50 p-4 -mx-4 rounded-lg transition-colors">
                      ...
                      <div className="flex items-center space-x-2">
                        <button onClick={() => handleQuantityChange(item._id, item.quantity - 1)} disabled={isLoading || item.quantity <= 1} className="p-1 border border-gray-300 rounded hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed">
                  ...
                  {items.length > 0 && filteredItems.length === 0 && (
                     <div className="text-center py-8">
                        <p className="text-gray-600">No items in your cart match "{cartSearchQuery}".</p>
                     </div>
                  )}
                </div>
              </div>
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-lg shadow-sm sticky top-8 p-6">
                ...
                {!discountCode ? (
                  <form onSubmit={handleApplyDiscount} className="mb-6 space-y-2">
                    ...
                    <div className="flex space-x-2">
                      ...
                      >
                        {isApplyingDiscount ? <LoadingSpinner size="sm" /> : <Tag className="h-4 w-4" />}
                      </button>
                    </div>
                  </form>
                ) : (
                ...
                <button onClick={handleCheckout} disabled={isLoading || items.length === 0} className="w-full bg-blue-600 text-white py-3 px-6 rounded-lg font-semibold hover:bg-blue-700 transition-colors flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed">
