...
import StockMovementsChart from '../../components/inventory/StockMovements';
...
const SupplierInventory = () => {
  ...
  // Fetch supplier's inventory summary
  ...
  // Fetch supplier's low stock products
  const { data: lowStockProducts, isLoading: lowStockLoading } = useQuery(
    'supplier-low-stock-products',
    async () => {
      const response = await axios.get('/inventory/supplier/low-stock');
      return response.data.products;
    }
  );

  // Fetch supplier's inventory items
  const { data: inventoryData, isLoading: inventoryLoading } = useQuery(
    ['supplier-inventory-items', searchQuery, filterStatus],
    async () => {
      const params = new URLSearchParams();
      if (searchQuery) params.append('search', searchQuery);
      if (filterStatus !== 'all') params.append('isLowStock', filterStatus === 'low');
      
      const response = await axios.get(`/inventory/supplier?${params.toString()}`);
      console.log('inventory: ', response.data)
      return response.data;
    }
  );

// Fetch supplier's stock movements
const { data: stockMovementsData, isLoading: movementsLoading } = useQuery(
  ['supplier-stock-movements', searchQuery, filterStatus],
  async () => {
    const params = new URLSearchParams();
    if (searchQuery) params.append('search', searchQuery); // optional: filter by product name
    if (filterStatus && filterStatus !== 'all') params.append('type', filterStatus); // optional: filter by movement type
    
    const response = await axios.get(`/inventory/supplier/movements?${params.toString()}`);
    console.log('stock movements: ', response.data);
    return response.data.movements; // return only movements array
  }
);
  ...
  const stockMutation = useMutation(
    async (data) => {
      ...
      if (action === 'add') {
        return axios.post(`/inventory/supplier/${productId}/add-stock`, {
          quantity,
          reason,
          cost,
          notes
        ...
        return axios.post(`/inventory/supplier/${productId}/remove-stock`, {
          quantity,
          reason,
          notes
        });
      } else if (action === 'adjust') {
        return axios.post(`/inventory/supplier/${productId}/adjust-stock`, {
    ...
    {
      onSuccess: () => {
        queryClient.invalidateQueries('supplier-inventory-summary');
        queryClient.invalidateQueries('supplier-low-stock-products');
        queryClient.invalidateQueries('supplier-inventory-items');
        queryClient.invalidateQueries('supplier-stock-movements');
  ...
  return (
    <>
      <Helmet>
        <title>My Inventory - Sportify Supplier</title>
        <meta name="description" content="Manage your product inventory on Sportify." />
      </Helmet>
              ...
              {activeTab === 'overview' && (
                <div className="space-y-6">
                  ...
                  <div>
                    ...
                    {!movementsLoading && stockMovementsData && (
                      <>
                        <StockMovementsChart movements={stockMovementsData} />
                      </>
                    )}
                          ...
                          <tr key={item._id} className="hover:bg-gray-50">
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                              <div className="flex space-x-2">
                                ...
                                <button
                                  onClick={() => setHistoryModal({ open: true, productId: item._id, productName: item.product?.name })}
