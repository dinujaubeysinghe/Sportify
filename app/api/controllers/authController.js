...
exports.register = async (req, res) => {
  try {
    ...
    if (role === 'supplier') {
      ...
      userData.isVerified = true; // Auto-verify all accounts
    } else if (role === 'staff') {
      userData.employeeId = employeeId;
      userData.department = department;
      userData.hireDate = hireDate;
      userData.isVerified = true; // Auto-verify staff accounts
    } else {
      userData.isVerified = true; // Auto-verify customer accounts
    }
...
exports.login = async (req, res) => {
  try {
    ...
    // Email verification removed for all users
    // if (!user.isVerified && user.role !== 'staff') return res.status(401).json({ success: false, message: 'Please verify your email before logging in' });
